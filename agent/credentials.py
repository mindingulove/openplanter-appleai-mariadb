...
class CredentialBundle:
    ...
    def has_any(self) -> bool:
        return bool(
            ...
            or (self.apple_api_key and self.apple_api_key.strip())
    ...
    def merge_missing(self, other: "CredentialBundle") -> None:
        ...
        if not self.apple_api_key and other.apple_api_key:
            self.apple_api_key = other.apple_api_key
    ...
    def to_json(self) -> dict[str, str]:
        ...
        if self.apple_api_key:
            out["apple_api_key"] = self.apple_api_key
        if self.exa_api_key:
    ...
    def from_json(cls, payload: dict[str, str] | None) -> "CredentialBundle":
        ...
        return cls(
            ...
            apple_api_key=(payload.get("apple_api_key") or "").strip() or None,
...
def parse_env_file(path: Path) -> CredentialBundle:
    ...
    return CredentialBundle(
        ...
        or None,
        apple_api_key=(env.get("APPLE_API_KEY") or env.get("OPENPLANTER_APPLE_API_KEY") or "").strip() or None,
...
def credentials_from_env() -> CredentialBundle:
    return CredentialBundle(
        ...
        or None,
        apple_api_key=(
            os.getenv("OPENPLANTER_APPLE_API_KEY") or os.getenv("APPLE_API_KEY") or ""
        ).strip()
        or None,
...
def prompt_for_credentials(
    ...
    current = CredentialBundle(
        ...
        apple_api_key=existing.apple_api_key,
