...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=None,
        help="Persist MariaDB integration as enabled by default.",
    )
    parser.add_argument(
        "--default-no-mariadb",
        action="store_false",
        dest="default_use_mariadb",
        help="Persist MariaDB integration as disabled by default.",
    )
    parser.add_argument(
        "--show-settings",
        action="store_true",
    ...
    parser.add_argument("--apple-api-key", help="Apple bridge API key override.")
    ...
    parser.add_argument("--demo", action="store_true", help="Censor entity names and workspace paths.")
    
    # MariaDB / MySQL Connection overrides
    parser.add_argument("--mariadb-host", help="MariaDB host override.")
    parser.add_argument("--mariadb-port", type=int, help="MariaDB port override.")
    parser.add_argument("--mariadb-user", help="MariaDB user override.")
    parser.add_argument("--mariadb-password", help="MariaDB password override.")
    parser.add_argument("--mariadb-database", "--db", help="MariaDB database override.")
    parser.add_argument(
        "--use-mariadb",
        action="store_true",
        default=None,
        help="Explicitly enable MariaDB integration.",
    )
    parser.add_argument(
        "--no-mariadb",
        action="store_false",
        dest="use_mariadb",
        help="Explicitly disable MariaDB integration.",
    )
...
def _apply_runtime_overrides(cfg: AgentConfig, args: argparse.Namespace, creds: CredentialBundle) -> None:
    ...
    cfg.apple_api_key = creds.apple_api_key
    ...
    if args.mariadb_host:
        cfg.mariadb_host = args.mariadb_host
    if args.mariadb_port:
        cfg.mariadb_port = args.mariadb_port
    if args.mariadb_user:
        cfg.mariadb_user = args.mariadb_user
    if args.mariadb_password:
...
def _apply_persistent_settings(
    ...
    # Apply base URLs from settings to config if they exist
    if settings.openai_base_url:
        cfg.openai_base_url = settings.openai_base_url
    if settings.anthropic_base_url:
        cfg.anthropic_base_url = settings.anthropic_base_url
    if settings.openrouter_base_url:
        cfg.openrouter_base_url = settings.openrouter_base_url
    if settings.cerebras_base_url:
        cfg.cerebras_base_url = settings.cerebras_base_url
    ...
    # Apply MariaDB settings from store to config
    if settings.mariadb_host:
        cfg.mariadb_host = settings.mariadb_host
    if settings.mariadb_port:
        try:
            cfg.mariadb_port = int(settings.mariadb_port)
        except (ValueError, TypeError):
            pass
    if settings.mariadb_user:
        cfg.mariadb_user = settings.mariadb_user
    if settings.mariadb_password:
        cfg.mariadb_password = settings.mariadb_password
    if settings.mariadb_database:
        cfg.mariadb_database = settings.mariadb_database
    
    if settings.use_mariadb is not None:
        cfg.use_mariadb = settings.use_mariadb.lower() in ("1", "true", "yes", "y")

    if args.use_mariadb is not None:
        cfg.use_mariadb = args.use_mariadb
...
def main() -> None:
    ...
    # Auto-discover the Apple bridge port AFTER all settings are applied
    cfg.discover_apple_bridge()
    ...
    startup_info: dict[str, str] = {
        ...
        "Model": model_name,
        "MariaDB (SQL)": f"{cfg.mariadb_database} [ACTIVE]" if cfg.use_mariadb else "[DISABLED]",
    }
